...
### Why This Tool Doesn't Work on macOS

macOS does not use systemd or journalctl for logging. Instead, it uses:
- **unified logging system** accessed via `log` command
- **ASL (Apple System Log)** accessed via `syslog` command (deprecated)

### Alternative Approach for macOS (Development/Testing Only)

You can simulate the functionality by modifying the code to use macOS logs:

#### Option 1: Use macOS `log stream`

Modify `JOURNALCTL_CMD` in `db.py`:

```python
# For macOS testing only - do NOT commit
JOURNALCTL_CMD = ["log", "stream", "--style", "syslog"]
```
...
Create a test script to simulate logs:

```bash
# test_mock_logs.sh
while true; do
  echo "$(date '+%b %d %H:%M:%S') testhost mockservice[$$]: Test log entry $RANDOM"
  sleep 0.5
done
```

Modify `db.py` to read from this script:

```python
# For macOS testing only
JOURNALCTL_CMD = ["bash", "test_mock_logs.sh"]
```
